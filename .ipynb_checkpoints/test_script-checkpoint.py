...
def testing(bot, job):
    bot.sendMessage(chat_id = job.context.message.chat_id ,text="still working")
    return()
    

def daily_job(bot, update, job_queue):
    bot.sendMessage(chat_id=update.message.chat_id, text="started")
    job_queue.run_daily(sendmessage,time=datetime.time(10,53,0,tzinfo=indt), context=update)
...
def main():
    ...
    dp.add_handler(CommandHandler('s', daily_job, pass_job_queue=True))
